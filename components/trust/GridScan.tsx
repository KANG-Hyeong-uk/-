...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (uLineStyle > 0.5) {
        ...
        if (uLineStyle < 1.5) {
            lineX *= dashMaskY; lineY *= dashMaskX;
        } else {
    ...
    if (uLineStyle > 0.5) {
        ...
        if (uLineStyle < 1.5) {
            lineX2 *= dashMaskY2; lineY2 *= dashMaskX2;
        } else {
...
interface SmoothDampVec2Result extends THREE.Vector2 {}
...
export function GridScan({
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      ...
      (bloom.blendMode.opacity as unknown as { value: number }).value = Math.max(0, bloomIntensity);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    lineJitter, scanDirection, enablePost, noiseIntensity, bloomIntensity, scanGlow,
    scanSoftness, scanPhaseTaper, scanDuration, scanDelay, bloomThreshold, bloomSmoothing,
    chromaticAberration, smoothTime, maxSpeed, skewScale, yBoost, tiltScale, yawScale,
  ]);

  // Uniform hot-update on prop changes
  useEffect(() => {
    ...
    if (bloomRef.current) {
      (bloomRef.current.blendMode.opacity as unknown as { value: number }).value = Math.max(0, bloomIntensity);
    ...
    bloomIntensity, bloomThreshold, bloomSmoothing, chromaticAberration, noiseIntensity,
    scanGlow, scanOpacity, scanDirection, scanSoftness, scanPhaseTaper, scanDuration, scanDelay,
  ]);
